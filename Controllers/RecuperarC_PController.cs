...
namespace PSP_.Controllers
{
    ...
    [ApiController]
    public class RecuperarC_PController : ControllerBase
    {
        ...
        public ActionResult Put([FromBody] Models.Usuario modelo)
        {
            ...
            using (Models.DBPSPPLUSContext db = new Models.DBPSPPLUSContext())
            {
                ...
                var usuario = (from d in db.Usuarios
                               select d).Where(d => d.Email == modelo.Email).ToList();

                foreach (var item in parametros)
                {
                    from = item.Correo;
                    ctp = DecryptPassword(item.Clave);
                }


                MailMessage message = new MailMessage(from, modelo.Email);
                ...
                System.Net.NetworkCredential(from, ctp);
